...
class XilinxZCU104MIGPads(depth : BigInt) extends ZCU104MIGIODDR(depth) {
  def this(c : XilinxZCU104MIGParams) {
    this(AddressRange.fromSets(c.address).head.size)
  }
}

class XilinxZCU104MIGIO(depth : BigInt) extends ZCU104MIGIODDR(depth) with ZCU104MIGIOClocksReset

class XilinxZCU104MIGIsland(c : XilinxZCU104MIGParams)(implicit p: Parameters) extends LazyModule with CrossesToOnlyOneClockDomain {
  val ranges = AddressRange.fromSets(c.address)
  require (ranges.size == 1, "DDR range must be contiguous")
  val offset = ranges.head.base
  val depth = ranges.head.size
  val crossing = AsynchronousCrossing(8)
  require((depth<=0x80000000L),"zcu104mig supports upto 4GB depth configuraton")
  ...
  lazy val module = new LazyModuleImp(this) {
    val io = IO(new Bundle {
      val port = new XilinxZCU104MIGIO(depth)
    })
...
class XilinxZCU104MIG(c : XilinxZCU104MIGParams)(implicit p: Parameters) extends LazyModule {
  val ranges = AddressRange.fromSets(c.address)
  val depth = ranges.head.size
  ...
  val island  = LazyModule(new XilinxZCU104MIGIsland(c))
