...
import sifive.fpgashells.devices.xilinx.xilinxzcu104mig_new._
...
class SysClockZCU104PlacedOverlay_new(val shell: ZCU104ShellBasicOverlays_new, name: String, val designInput: ClockInputDesignInput, val shellInput: ClockInputShellInput)
...
class SysClockZCU104ShellPlacer_new(shell: ZCU104ShellBasicOverlays_new, val shellInput: ClockInputShellInput)(implicit val valName: ValName)
  extends ClockInputShellPlacer[ZCU104ShellBasicOverlays_new]
{
    def place(designInput: ClockInputDesignInput) = new SysClockZCU104PlacedOverlay_new(shell, valName.name, designInput, shellInput)
}

// ZCU104 DDR clock @ 300MHz
class DDRClockZCU104PlacedOverlay_new(val shell: ZCU104ShellBasicOverlays_new, name: String, val designInput: ClockInputDesignInput, val shellInput: ClockInputShellInput)
  extends LVDSClockInputXilinxPlacedOverlay(name, designInput, shellInput)
{
...
class DDRClockZCU104ShellPlacer_new(shell: ZCU104ShellBasicOverlays_new, val shellInput: ClockInputShellInput)(implicit val valName: ValName)
  extends ClockInputShellPlacer[ZCU104ShellBasicOverlays_new]
{
    def place(designInput: ClockInputDesignInput) = new DDRClockZCU104PlacedOverlay_new(shell, valName.name, designInput, shellInput)
}


class RefClockZCU104PlacedOverlay_new(val shell: ZCU104ShellBasicOverlays_new, name: String, val designInput: ClockInputDesignInput, val shellInput: ClockInputShellInput)
  extends LVDSClockInputXilinxPlacedOverlay(name, designInput, shellInput) {
  val node = shell { ClockSourceNode(freqMHz = 125, jitterPS = 50)(ValName(name)) }
...
class RefClockZCU104ShellPlacer_new(shell: ZCU104ShellBasicOverlays_new, val shellInput: ClockInputShellInput)(implicit val valName: ValName)
  extends ClockInputShellPlacer[ZCU104ShellBasicOverlays_new] {
  def place(designInput: ClockInputDesignInput) = new RefClockZCU104PlacedOverlay_new(shell, valName.name, designInput, shellInput)
}
...
// Placed on PMOD0
class SDIOZCU104PlacedOverlay_new(val shell: ZCU104ShellBasicOverlays_new, name: String, val designInput: SPIDesignInput, val shellInput: SPIShellInput)
...
class SDIOZCU104ShellPlacer_new(shell: ZCU104ShellBasicOverlays_new, val shellInput: SPIShellInput)(implicit val valName: ValName)
  extends SPIShellPlacer[ZCU104ShellBasicOverlays_new] {
  def place(designInput: SPIDesignInput) = new SDIOZCU104PlacedOverlay_new(shell, valName.name, designInput, shellInput)
}

class SPIFlashZCU104PlacedOverlay_new(val shell: ZCU104ShellBasicOverlays_new, name: String, val designInput: SPIFlashDesignInput, val shellInput: SPIFlashShellInput)
...
class SPIFlashZCU104ShellPlacer_new(shell: ZCU104ShellBasicOverlays_new, val shellInput: SPIFlashShellInput)(implicit val valName: ValName)
  extends SPIFlashShellPlacer[ZCU104ShellBasicOverlays_new] {
  def place(designInput: SPIFlashDesignInput) = new SPIFlashZCU104PlacedOverlay_new(shell, valName.name, designInput, shellInput)
}

class UARTZCU104PlacedOverlay_new(val shell: ZCU104ShellBasicOverlays_new, name: String, val designInput: UARTDesignInput, val shellInput: UARTShellInput)
...
class UARTZCU104ShellPlacer_new(shell: ZCU104ShellBasicOverlays_new, val shellInput: UARTShellInput)(implicit val valName: ValName)
  extends UARTShellPlacer[ZCU104ShellBasicOverlays_new] {
  def place(designInput: UARTDesignInput) = new UARTZCU104PlacedOverlay_new(shell, valName.name, designInput, shellInput)
}

// NOTE: ZCU104 doesn't have QSFP
class QSFP1ZCU104PlacedOverlay_new(val shell: ZCU104ShellBasicOverlays_new, name: String, val designInput: EthernetDesignInput, val shellInput: EthernetShellInput)
...
class QSFP1ZCU104ShellPlacer_new(shell: ZCU104ShellBasicOverlays_new, val shellInput: EthernetShellInput)(implicit val valName: ValName)
  extends EthernetShellPlacer[ZCU104ShellBasicOverlays_new] {
  def place(designInput: EthernetDesignInput) = new QSFP1ZCU104PlacedOverlay_new(shell, valName.name, designInput, shellInput)
}

// NOTE: ZCU104 doesn't have QSFP
class QSFP2ZCU104PlacedOverlay_new(val shell: ZCU104ShellBasicOverlays_new, name: String, val designInput: EthernetDesignInput, val shellInput: EthernetShellInput)
  extends EthernetUltraScalePlacedOverlay(name, designInput, shellInput, XXVEthernetParams(name = name, speed   = 10, dclkMHz = 125))
{
  // val dclkSource = shell { BundleBridgeSource(() => Clock()) }
  // val dclkSink = dclkSource.makeSink()
...
class QSFP2ZCU104ShellPlacer_new(shell: ZCU104ShellBasicOverlays_new, val shellInput: EthernetShellInput)(implicit val valName: ValName)
  extends EthernetShellPlacer[ZCU104ShellBasicOverlays_new] {
  def place(designInput: EthernetDesignInput) = new QSFP2ZCU104PlacedOverlay_new(shell, valName.name, designInput, shellInput)
}

object LEDZCU104PinConstraints_new {
  val pins = Seq("D5", "D6", "A5", "B5")
}
class LEDZCU104PlacedOverlay_new(val shell: ZCU104ShellBasicOverlays_new, name: String, val designInput: LEDDesignInput, val shellInput: LEDShellInput)
  extends LEDXilinxPlacedOverlay(name, designInput, shellInput, packagePin = Some(LEDZCU104PinConstraints_new.pins(shellInput.number)), ioStandard = "LVCMOS12")
class LEDZCU104ShellPlacer_new(shell: ZCU104ShellBasicOverlays_new, val shellInput: LEDShellInput)(implicit val valName: ValName)
  extends LEDShellPlacer[ZCU104ShellBasicOverlays_new] {
  def place(designInput: LEDDesignInput) = new LEDZCU104PlacedOverlay_new(shell, valName.name, designInput, shellInput)
}

object ButtonZCU104PinConstraints_new {
  val pins = Seq("B4", "C4", "B3", "C3")
}
class ButtonZCU104PlacedOverlay_new(val shell: ZCU104ShellBasicOverlays_new, name: String, val designInput: ButtonDesignInput, val shellInput: ButtonShellInput)
  extends ButtonXilinxPlacedOverlay(name, designInput, shellInput, packagePin = Some(ButtonZCU104PinConstraints_new.pins(shellInput.number)), ioStandard = "LVCMOS18")
class ButtonZCU104ShellPlacer_new(shell: ZCU104ShellBasicOverlays_new, val shellInput: ButtonShellInput)(implicit val valName: ValName)
  extends ButtonShellPlacer[ZCU104ShellBasicOverlays_new] {
  def place(designInput: ButtonDesignInput) = new ButtonZCU104PlacedOverlay_new(shell, valName.name, designInput, shellInput)
}

object SwitchZCU104PinConstraints_new {
  val pins = Seq("E4", "C4", "F5", "F4")
}
class SwitchZCU104PlacedOverlay_new(val shell: ZCU104ShellBasicOverlays_new, name: String, val designInput: SwitchDesignInput, val shellInput: SwitchShellInput)
  extends SwitchXilinxPlacedOverlay(name, designInput, shellInput, packagePin = Some(SwitchZCU104PinConstraints_new.pins(shellInput.number)), ioStandard = "LVCMOS12")
class SwitchZCU104ShellPlacer_new(shell: ZCU104ShellBasicOverlays_new, val shellInput: SwitchShellInput)(implicit val valName: ValName)
  extends SwitchShellPlacer[ZCU104ShellBasicOverlays_new] {
  def place(designInput: SwitchDesignInput) = new SwitchZCU104PlacedOverlay_new(shell, valName.name, designInput, shellInput)
}

//NOTE: Do we need chiplink on the FMC connector?
class ChipLinkZCU104PlacedOverlay_new(val shell: ZCU104ShellBasicOverlays_new, name: String, val designInput: ChipLinkDesignInput, val shellInput: ChipLinkShellInput)
...
class ChipLinkZCU104ShellPlacer_new(shell: ZCU104ShellBasicOverlays_new, val shellInput: ChipLinkShellInput)(implicit val valName: ValName)
  extends ChipLinkShellPlacer[ZCU104ShellBasicOverlays_new] {
  def place(designInput: ChipLinkDesignInput) = new ChipLinkZCU104PlacedOverlay_new(shell, valName.name, designInput, shellInput)
}

// TODO: JTAG is untested
// NOTE: Commented out for ZCU104
// Placed on PMOD1
class JTAGDebugZCU104PlacedOverlay_new(val shell: ZCU104ShellBasicOverlays_new, name: String, val designInput: JTAGDebugDesignInput, val shellInput: JTAGDebugShellInput)
...
class JTAGDebugZCU104ShellPlacer_new(shell: ZCU104ShellBasicOverlays_new, val shellInput: JTAGDebugShellInput)(implicit val valName: ValName)
  extends JTAGDebugShellPlacer[ZCU104ShellBasicOverlays_new] {
  def place(designInput: JTAGDebugDesignInput) = new JTAGDebugZCU104PlacedOverlay_new(shell, valName.name, designInput, shellInput)
}

// NOTE: Commented out for ZCU104
class cJTAGDebugZCU104PlacedOverlay_new(val shell: ZCU104ShellBasicOverlays_new, name: String, val designInput: cJTAGDebugDesignInput, val shellInput: cJTAGDebugShellInput)
...
class cJTAGDebugZCU104ShellPlacer_new(shell: ZCU104ShellBasicOverlays_new, val shellInput: cJTAGDebugShellInput)(implicit val valName: ValName)
  extends cJTAGDebugShellPlacer[ZCU104ShellBasicOverlays_new] {
  def place(designInput: cJTAGDebugDesignInput) = new cJTAGDebugZCU104PlacedOverlay_new(shell, valName.name, designInput, shellInput)
}

class JTAGDebugBScanZCU104PlacedOverlay_new(val shell: ZCU104ShellBasicOverlays_new, name: String, val designInput: JTAGDebugBScanDesignInput, val shellInput: JTAGDebugBScanShellInput)
  extends JTAGDebugBScanXilinxPlacedOverlay(name, designInput, shellInput)
class JTAGDebugBScanZCU104ShellPlacer_new(val shell: ZCU104ShellBasicOverlays_new, val shellInput: JTAGDebugBScanShellInput)(implicit val valName: ValName)
  extends JTAGDebugBScanShellPlacer[ZCU104ShellBasicOverlays_new] {
  def place(designInput: JTAGDebugBScanDesignInput) = new JTAGDebugBScanZCU104PlacedOverlay_new(shell, valName.name, designInput, shellInput)
}

case object ZCU104DDRSize_new extends Field[BigInt](0x40000000L * 8) // 2GB
class DDRZCU104PlacedOverlay_new(val shell: ZCU104ShellBasicOverlays_new, name: String, val designInput: DDRDesignInput, val shellInput: DDRShellInput)
  extends DDRPlacedOverlay[XilinxZCU104MIGPads_new](name, designInput, shellInput)
{
  val size = p(ZCU104DDRSize_new)

  val migParams = XilinxZCU104MIGParams_new(address = AddressSet.misaligned(di.baseAddress, size))
  val mig = LazyModule(new XilinxZCU104MIG_new(migParams))
  ...
  def ioFactory = new XilinxZCU104MIGPads_new(size)

  InModuleBody { ioNode.bundle <> mig.module.io }

  shell { InModuleBody {
    require (shell.ddr_clock.get.isDefined, "Use of DDRZCU104Overlay_new depends on DDRClockZCU104Overlay_new")
...
class DDRZCU104ShellPlacer_new(shell: ZCU104ShellBasicOverlays_new, val shellInput: DDRShellInput)(implicit val valName: ValName)
  extends DDRShellPlacer[ZCU104ShellBasicOverlays_new] {
  def place(designInput: DDRDesignInput) = new DDRZCU104PlacedOverlay_new(shell, valName.name, designInput, shellInput)
}
...
abstract class ZCU104ShellBasicOverlays_new()(implicit p: Parameters) extends UltraScaleShell{
  // PLL reset causes
  val pllReset = InModuleBody { Wire(Bool()) }

  val ddr_clock = Overlay(ClockInputOverlayKey, new DDRClockZCU104ShellPlacer_new(this, ClockInputShellInput()))
  val sys_clock = Overlay(ClockInputOverlayKey, new SysClockZCU104ShellPlacer_new(this, ClockInputShellInput()))
  ...
  val led       = Seq.tabulate(8)(i => Overlay(LEDOverlayKey, new LEDZCU104ShellPlacer_new(this, LEDShellInput(color = "red", number = i))(valName = ValName(s"led_$i"))))
  val switch    = Seq.tabulate(4)(i => Overlay(SwitchOverlayKey, new SwitchZCU104ShellPlacer_new(this, SwitchShellInput(number = i))(valName = ValName(s"switch_$i"))))
  val button    = Seq.tabulate(5)(i => Overlay(ButtonOverlayKey, new ButtonZCU104ShellPlacer_new(this, ButtonShellInput(number = i))(valName = ValName(s"button_$i"))))
  val ddr       = Overlay(DDROverlayKey, new DDRZCU104ShellPlacer_new(this, DDRShellInput()))
  ...
  val chiplink  = Overlay(ChipLinkOverlayKey, new ChipLinkZCU104ShellPlacer_new(this, ChipLinkShellInput()))
  //val spi_flash = Overlay(SPIFlashOverlayKey, new SPIFlashZCU104ShellPlacer(this, SPIFlashShellInput()))
  //SPI Flash not functional
}

case object ZCU104ShellPMOD_new extends Field[String]("JTAG")
case object ZCU104ShellPMOD2_new extends Field[String]("JTAG")

class WithZCU104ShellPMOD_new(device: String) extends Config((site, here, up) => {
  case ZCU104ShellPMOD_new => device
})
...
class WithZCU104ShellPMOD2_new(device: String) extends Config((site, here, up) => {
  case ZCU104ShellPMOD2_new => device
})

class WithZCU104ShellPMODJTAG_new extends WithZCU104ShellPMOD_new("JTAG")
class WithZCU104ShellPMODSDIO_new extends WithZCU104ShellPMOD_new("SDIO")

// Reassign JTAG pinouts location to PMOD J53
class WithZCU104ShellPMOD2JTAG_new extends WithZCU104ShellPMOD2_new("PMODJ53_JTAG")

class ZCU104Shell_new()(implicit p: Parameters) extends ZCU104ShellBasicOverlays_new
{
  val pmod_is_sdio  = p(ZCU104ShellPMOD_new) == "SDIO"
  ...
  val uart      = Overlay(UARTOverlayKey, new UARTZCU104ShellPlacer_new(this, UARTShellInput()))
  val sdio      = if (pmod_is_sdio) Some(Overlay(SPIOverlayKey, new SDIOZCU104ShellPlacer_new(this, SPIShellInput()))) else None

  //NOTE: ZCU104 commented because we don't need it
  val jtag      = Overlay(JTAGDebugOverlayKey, new JTAGDebugZCU104ShellPlacer_new(this, JTAGDebugShellInput()))
  // val cjtag     = Overlay(cJTAGDebugOverlayKey, new cJTAGDebugZCU104ShellPlacer(this, cJTAGDebugShellInput()))
  val jtagBScan = Overlay(JTAGDebugBScanOverlayKey, new JTAGDebugBScanZCU104ShellPlacer_new(this, JTAGDebugBScanShellInput()))
  ...
  override lazy val module = new LazyRawModuleImp(this) {
    ...
    val sysclk: Clock = sys_clock.get() match {
      case Some(x: SysClockZCU104PlacedOverlay_new) => x.clock
    }
    ...
    val ereset: Bool = chiplink.get() match {
      case Some(x: ChipLinkZCU104PlacedOverlay_new) => !x.ereset_n
      case _ => false.B
    }
