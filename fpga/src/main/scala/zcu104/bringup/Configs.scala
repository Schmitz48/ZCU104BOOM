...
import sifive.fpgashells.shell.xilinx.{VCU118ShellPMOD, ZCU104DDRSize}

import testchipip.{PeripheryTSIHostKey, TSIHostParams, TSIHostSerdesParams}

import chipyard.{BuildSystem}

import chipyard.fpga.zcu104.{WithZCU104Tweaks, WithFPGAFrequency, ZCU104DDR2Size}
  ...
  case PeripheryTSIHostKey => List(
    TSIHostParams(
      ...
      targetMasterPortParams = MasterPortParams(
        ...
        size = site(ZCU104DDR2Size),
        beatBytes = 8, // comes from test chip
        idBits = 4) // comes from VCU118 idBits in XilinxVCU118MIG
      ))
})

class WithBringupZCU104System extends Config((site, here, up) => {
  case BuildSystem => (p: Parameters) => new BringupZCU104DigitalTop()(p) // use the VCU118-extended bringup digital top
})
